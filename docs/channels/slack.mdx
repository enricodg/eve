...
Function-form tokens receive an optional `SlackBotTokenContext` —
`{ teamId, channelId, threadTs }` — populated wherever the channel knows
which workspace it is acting for: inbound bindings, interaction updates,
inbound file staging, and proactive sessions. A multi-workspace
installation (one distributed Slack app installed by many teams) can
resolve each workspace's own token without Vercel Connect:
...
export default slackChannel({
  credentials: {
    botToken: async (context) => lookupWorkspaceToken(context?.teamId), // your own store
  },
});
```

Zero-argument resolvers keep working unchanged, and string tokens ignore
...
Start a session without an inbound message through `receive(slack, { message, target, auth })` from a schedule `run` handler, or `args.receive(slack, ...)` from another channel. The proactive target shape is `{ channelId, teamId? }` — set `teamId` in multi-workspace installations so the session's posts resolve the owning workspace's token.
